...
このリポジトリには、コードの品質と信頼性を保証するための自動テストとシェルスクリントのチェックが含まれています。

### 自動テストの実行

テストスイートを実行するには、以下のスクリプトを実行してください:
...
メインスクリプトに潜在的な問題がないかShellCheckで確認するには、以下を実行します:
```bash
shellcheck bin/usb-wakeup-blocker.sh
```
何も出力されなければ、リントの問題はありません。

**注:** これらのチェックは、変更をプッシュしたりプルリクエストを開いたりした際に、GitHub Actionsでも自動的に実行されます。
